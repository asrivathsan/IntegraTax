...
const CONTAINER_RIGHT_EDGE = containerWidth - margin.right;
...
const xlinear = v => LEFT_PAD + (v / maxXV) * usablewidth;
  ...
  if (window.spartActive && window.universalRuns) {
  drawSPARTBars(svg, treeData,CONTAINER_RIGHT_EDGE);
} else {
...
function precollapseatthreshold(treeData, threshold) {if (!treeData) return;
  collapsedbands.length = 0;
  treeData.descendants().forEach(n => {n._collapsedV = false;
    n._hiddenV = false;});
  treeData.descendants().forEach(n => {if (!n.data.terminal &&
      parseFloat(n.data.fusepoint) <= threshold &&!n.data.frozen && !hasFrozenDescendant(n)) {let ancestorcollapsed = false,
      p = n.parent;
      while (p) {
        if (p._collapsedV) {ancestorcollapsed = true;
          break;}
        p = p.parent;}
      if (!ancestorcollapsed) {n._collapsedV = true;
