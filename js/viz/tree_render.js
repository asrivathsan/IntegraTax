...
function openfile(event){window.input=event.target||document.getElementById("myFile");
...
function updateTerminalLabelLength(maxLen) {
  maxnameLength = maxLen;
  window.maxKeep = maxLen;

	Object.keys(terminallabel).forEach(key => {
	  terminallabelshort[key] = shortenspecimen(terminallabel[key], maxLen);
	});

	for (const term of Object.keys(terminallabel)) {
	  const el = document.getElementById("terminaltext" + term + "id");
	  if (!el) continue;
  ...
  reflowx_log();
	...
	function cleanthreshold(val) {return parseFloat(val.split(" ")[0]);}
