...
function updatenodeColour(invar) {
  ...
  if (invar == "PI/nonPI"){var LITmax=parseFloat(document.getElementById("LITmax").value);
    ...
    var nodes=treeData.descendants();
      nodes.forEach(d=>{if (d.data.frozen) return;if (d.data.nodestat==="v") return;if (d.data.frozen) return;if (d.data.fusepoint>LITmax){d.data.linkcolor=false;
      ...
      nodes.forEach(d=>{if (d.data.frozen) return;if (d.data.nodestat==="v") return;if (d.data.terminal){if(!highlightnodesnonPI.includes(d)&&!highlightnodesPI.includes(d)) {
        d.data.PIstat=0;
        d.data.nodestat="nnpi";
        d3.select("#circleid"+d.data.terminal+"id").style("fill", "#1F83F3");}}});

      nodes.forEach(d=>{if (d.data.frozen) return;if (d.data.nodestat==="v") return;if (highlightnodesPI.includes(d)){d3.select("#circleid"+d.data.name+"id").style("fill", "#ff0000");
        d3.select("#circleid"+d.data.terminal+"id").style("fill", "#ff0000");
        if(d.data.terminal){d.data.PIstat=1;}}
  ...
  if (invar == "Binomial name") {var LITmax=parseFloat(document.getElementById("Clustperc").value);
    ...
    var nodes=treeData.descendants();
    nodes.forEach(d=>{if (d.data.nodestat==="v") return;if (d.data.frozen) return;var terminals=getTerminals(d);
      var termnames=[];
...
function updatesubtreeColour(invar) {if (invar == "None") {  return;}
  if (invar == "PI/nonPI") {
    ...
    var highlightnodesnonPI=[];
    var nodes=treeData.descendants();
    nodes.forEach(d => {if (d.data.nodestat==="v") return;if (d.data.frozen) return;if (d.data.fusepoint > LITmax) { d.data.linkcolor=true;
      d3.select("#circleid"+d.data.name+"id").style("fill", "#FFFFFF");
      d3.select("#circleid"+d.data.terminal+"id").style("fill", "#FFFFFF");} 
    ...
    nodes.forEach(d => {if (d.data.nodestat==="v") return;if (d.data.frozen) return;if (d.data.terminal) {if (!highlightnodesnonPI.includes(d)&&!highlightnodesPI.includes(d)) {d.data.nodestat="snpi";
     d3.select("#circleid"+d.data.terminal+"id").style("fill", "#1F83F3");}}});

    nodes.forEach(d => {if (d.data.nodestat==="v") return;if (d.data.frozen) return;if (highlightnodesPI.includes(d)) {d3.select("#circleid"+d.data.name+"id").style("fill", "#ff0000");
      d3.select("#circleid"+d.data.terminal+"id").style("fill", "#ff0000");}
      if (highlightnodesnonPI.includes(d)) {d3.select("#circleid"+d.data.name+"id").style("fill", "#1F83F3");
      d3.select("#circleid"+d.data.terminal+"id").style("fill", "#1F83F3");}});
  ...
  if (invar == "Binomial name") {var LITmax=parseFloat(document.getElementById("Clustperc").value);
    ...
    nodes.forEach(d => {if (d.data.nodestat==="v") return;if (d.data.frozen) return;var terminals=getTerminals(d);
      ...
      if (d.data.fusepoint > LITmax) {d3.select("#circleid"+d.data.name+"id").style("fill", "#FFFFFF");
          ...
          let childnodes=getDescendants(d);
          childnodes.forEach(k => {if (k != d) {highlightnodeslump.push(k);}});}}}); 
      nodes.forEach(d => {if (d.data.nodestat==="v") return;if (d.data.frozen) return;if (highlightnodescongruent.includes(d)) {d.data.nodestat="c";
        d3.select("#circleid"+d.data.name+"id").style("fill", "#008000");
        d3.select("#circleid"+d.data.terminal+"id").style("fill", "#008000");}
...
function updateterminalnodeColour(invar) {
  ...
  if (invar=="Binomial name"){clustercolors=clustcolors(spname);
    var nodes=treeData.descendants();
    var node=svg.selectAll("g.node").data(nodes,function(d){if (d.data.frozen === true ||d.data.nodestat === "v" ||d.data.textstat === "v" ||d.data.textstat === "va") return;try{var termname=d.data.terminal;
  ...
  if (invar=="LIT selections"){var LITmaxvalue=parseFloat(document.getElementById("LITmax").value);
    ...
    var nodes=treeData.descendants();
    nodes.forEach(d => {if (d.data.frozen === true ||d.data.nodestat === "v" ||d.data.textstat === "v" ||d.data.textstat === "va") return;d3.select("#circleid"+d.data.terminal+"id").style("fill", function(d){if(classifiedLITterminals[d.data.terminal]==1){return "#ff0000"}
    else{return "#ffffff"}});});}
}
...
function colorlinksbasedonnodes(instant = false) {
  function linkIsFrozen(d) {return (
      (d.source?.data?.frozen === true) ||
      (d.target?.data?.frozen === true));}

  var links = treeData.links();
  svg.selectAll("path.link")
    .each(function(d) {
      if (linkIsFrozen(d)) return;
      d3.select(this)
        .interrupt()
        .style("stroke", "#000000")
  ...
  svg.selectAll("path.link").data(links)  .each(function(d) {
    if (linkIsFrozen(d)) return; 
    d3.select(this)
    .transition(tt)
...
function spartColor(uid) {
  const frac =window.universalsupport[uid] / window.nspartschemes;
  return d3.interpolateGreens(frac * 0.9);}

function drawSPARTBars(svg, treeData,CONTAINER_RIGHT_EDGE) {
  if (!window.spartActive || !window.universalRuns?.length) {svg.selectAll(".spart-bar-group").remove();
    return;}
  svg.selectAll(".spart-bar-group").remove();
  const yPos = {};
  treeData.leaves().forEach(n => {
    const tid = n.data.terminal;
    const label = window.terminallabel?.[tid] ?? tid;
    yPos[label.toLowerCase()] = n.x;});
  let labelRightX = 0;
  svg.selectAll('g.node text[id^="terminaltext"]').each(function () {try {
      const text = this;
      ...
      if (right > labelRightX) labelRightX = right;} catch (e) {}
  });
  const PAD = 12;
const laneX =CONTAINER_RIGHT_EDGE -window.spartLanePx -SPART_LANE_PAD;
  let maxXUsed = laneX;
  const uidCounts = {};
  window.universalRuns.forEach(r => { uidCounts[r.uid] = (uidCounts[r.uid] || 0) + 1;});
  const g = svg.append("g").attr("class", "spart-bar-group");

  const BAR_PAD_Y = 6;
  window.universalRuns.forEach(run => {const y1 = yPos[run.members[0].toLowerCase()];
    const y2 = yPos[run.members.at(-1).toLowerCase()];
    if (y1 == null || y2 == null) return;
    const support = window.universalSupport[run.uid] || 0;
    const frac = support / window.nSpartSchemes;
    const barWidth = Math.max(2, frac * SPART_BAR_WIDTH);
    const x = laneX + run.col * (SPART_BAR_WIDTH + SPART_BAR_GAP);
    const rightEdge = x + barWidth;
if (rightEdge > maxXUsed) maxXUsed = rightEdge;
    console.log("draw x", x + barWidth);

    const yTop = Math.min(y1, y2) - BAR_PAD_Y;
    const height = Math.abs(y2 - y1) + 2 * BAR_PAD_Y;
    const isSplit = uidCounts[run.uid] > 1;
    ...
    barG.append("rect")
      ...
      .attr("width", barWidth)
      .attr("height", height)
      .attr("fill", d3.interpolateGreens(frac))
.attr("stroke", "#333")
.attr("stroke-width", 1)
      .attr("shape-rendering", "crispEdges")
      .on("mouseenter", function (event) {
        showTooltip(`<b>Universal set ${run.uid}</b><br>
          Support: ${support}/${window.nSpartSchemes}<br>
          ${isSplit? "<span style='color:#c00'><b>Split in dendrogram</b></span>": "Not split"} `);}).on("mousemove", function (event) { moveTooltip(event);}).on("mouseleave", function () {hideTooltip();});
      if (isSplit) {barG.append("rect")
        .attr("x", x + 2)
        .attr("y", yTop + 2)
        .attr("width", Math.max(1, barWidth - 4))
        .attr("height", Math.max(1, height - 4))
        .attr("fill", "none")
        .attr("stroke", "#000")
